...
impl <'a> Parser <'a> {
    fn get_next_token(&mut self) -> Result<(), ParseError> {
        let next_token = match  self.tokenizer.next() {
            Some(token) => token,
            None => return Err(ParseError::InvalidOperator("Invalid operator".into())),
        };
    ...
    fn generate_ast(&mut self, oper_prec: OperPrec) -> Result<Node, ParseError> {
        let mut letf_expr = self.parse_number()?;
        while oper_prec < self.current_token.get_oper_prec() {
            if self.current_token == Token::EOF {
                break;
            }
            let right_expr = self.convert_token_to_node(letf_expr.clone())?;
            letf_expr = right_expr;
        }
        Ok(letf_expr)
    }
...
impl fmt::Display for ParseError {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result{
        match &self {
            self::ParseError::UnableToParse(e) => write!(f, "Unable to parse {}", e),
            self::ParseError::InvalidOperator(e) => write!(f, "Error in evulating {}", e),
        }
    }
}
